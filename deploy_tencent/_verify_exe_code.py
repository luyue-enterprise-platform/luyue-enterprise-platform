...
# v2.3.8 起 onedir：主 EXE 位于 dist\鲁岳企业服务_综合智能平台\ 下
...
CHECKS = {
    ...
    'modules.insurance.blueprint': [
        ...
        'OCR识别阶段',
        '全程总计',
        # v2.3.8 台账任务独立目录（日志字面量针刺——区分 outputs 根目录旧版）
        '台账写入任务独立目录',
    ],
    ...
    'modules.pdf2word.blueprint': [
        ...
        '一个文件夹选择',
        '请移除后重新选择文件夹',
        # v2.3.8 上传清单手动排序（order 字段，docstring 针刺）
    ...
    # v2.5.0 第三方应用授权直连（OAuth 2.0 授权码 + PKCE S256，docstring/字面量针刺）
    'modules.mcp.core.oauth': [
        '第三方应用授权直连（v2.5.0）',
        '读取花名册解析结果（永不开放）',
    ...
    'modules.mcp.blueprint': [
        ...
        'MCP 访问日志',
        'mcp_access.log',
        # v2.5.0 授权直连路由与 scope 拦截（路由装饰器参数/字面量针刺）
        '双通道鉴权（v2.5.0）',
        '/oauth/authorize',
        '/oauth/token',
        '该工具不开放给第三方应用授权通道',
        '授权范围不足，请在应用授权管理中扩权后重新授权',
    ],
    ...
    'modules.mcp.core.tools': [
        ...
        '进入 Server 端等待',
        '退出等待',
        # v2.3.5 社保两阶段预览确认（字面量 + docstring 针刺）
        '核算预览已生成，尚未开始识别，等待确认参数与文件范围',
        '仅 waiting_confirm（待确认）状态可确认执行',
        # v2.3.8 强制两阶段（四工具）+ 返回安全化（字面量/docstring 针刺）
        '强制两阶段确认（v2.3.8）',
        '不得未经用户确认直接调用',
        '不得由调用方代替用户决定',
    ...
    'modules.mcp.core.adapters': [
        ...
        '按确认选择执行合同重命名',
        'waiting_confirm',
        # v2.3.5 社保两阶段预览：份数清点 + 花名册概览 + 预览暂存参数
        # （v2.3.6 起 MCP 层不再承载命名解析/匹配，针刺同步收敛）
        '参保证明清点（纯路径/扩展名，不做 OCR、不解析命名）',
        '任务缺少待确认的预览数据',
        # v2.3.8 摘要安全化：company_name 不进返回（docstring 针刺）
        '摘要不再包含 company_name',
    ],
