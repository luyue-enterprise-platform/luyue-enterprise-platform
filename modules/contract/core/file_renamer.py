...
def rename_contract_images(file_paths, roster, output_dir):
    """按花名册顺序对劳动合同图片重命名
    
    命名规则：
    - 单人单图: {序号:02d}-{姓名}.{ext}
    - 单人���图: {序号:02d}-{姓名}(1).{ext}, {序号:02d}-{姓名}(2).{ext}, ...
    - 未匹配的图片放入"未匹配"子文件夹
    ...
    # 提取文件名中的姓名（去除扩展名和路径）
    file_info = []  # [(original_path, original_name, guessed_name), ...]
    ...
    for fp in file_paths:
        ...
        file_info.append((fp, basename, guessed_name))
    
    # 按花名册分组：每个花名册人员对应哪些文件
    person_files = {}  # {seq: [file_info]}
    unmatched = []
    
    for fp, basename, guessed_name in file_info:
        matched = False
        # 精确匹配
